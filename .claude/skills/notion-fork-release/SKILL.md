...
- **<commit subject>** (#<PR>) — affects `<package>`
- ...
...
- **PR squashes happen separately**: this skill assumes patches are already on the `notion` branch. If they aren't, squash-merge them first (`git merge --squash <pr-branch>` then commit), then start at step 1.
